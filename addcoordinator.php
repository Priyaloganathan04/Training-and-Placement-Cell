...
if(isset($_POST)) {
	...
	$coordinatorname = mysqli_real_escape_string($conn, $_POST['coordinatorname']);
	...
	$sql = "SELECT email FROM coordinator WHERE email='$email'";
	...
	if($result->num_rows == 0) {
		...
		if($uploadOk == false) {
			header("Location: register-coordinator.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO coordinator( id, name, coordinatorname, country, state, city, contactno, dept, email, password, logo) VALUES ('$rid','$name', '$coordinatorname', '$country', '$state', '$city', '$contactno', '$dept', '$email', '$password', '$file')";

		if($conn->query($sql)===TRUE) {

			//If data inserted successfully then Set some session variables for easy reference and redirect to coordinator login
