...
if (isset($_POST)) {
	...
	$adharno = mysqli_real_escape_string($conn, $_POST['ano']);
	$gender = mysqli_real_escape_string($conn, $_POST['gender']);
	$dob = mysqli_real_escape_string($conn, $_POST['dob']);
	$age = mysqli_real_escape_string($conn, $_POST['age']);
	$assistance = mysqli_real_escape_string($conn, $_POST['assistance']);
	...
	if ($result->num_rows == 0) {
		...
		$sql = "INSERT INTO users(rno, fname, lname, email, ano, gender, dob, age, assistance, address, city, pincode, state, contactno, passingyear, dept, password, resume, hash ) VALUES ('$rno', '$fname', '$lname', '$email', '$adharno', '$gender', '$dob', '$age', '$assistance', '$address', '$city', '$pincode', '$state', '$contactno', '$passingyear', '$dept', '$password', '$file', '$hash')";
