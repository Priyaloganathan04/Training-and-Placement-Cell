...
$sql = "SELECT * FROM mailbox WHERE id_mailbox='$_GET[id_mail]' AND (id_fromuser='$_SESSION[id_coordinator]' OR id_touser='$_SESSION[id_coordinator]')";
$result = $conn->query($sql);
if ($result->num_rows >  0) {
  $row = $result->fetch_assoc();
  if ($row['fromuser'] == "coordinator") {
    $sql1 = "SELECT * FROM coordinator WHERE id_coordinator='$row[id_fromuser]'";
    $result1 = $conn->query($sql1);
    if ($result1->num_rows >  0) {
      $rowcoordinator = $result1->fetch_assoc();
    }
    ...
    $sql1 = "SELECT * FROM coordinator WHERE id_coordinator='$row[id_touser]'";
    $result1 = $conn->query($sql1);
    if ($result1->num_rows >  0) {
      $rowcoordinator = $result1->fetch_assoc();
    }
