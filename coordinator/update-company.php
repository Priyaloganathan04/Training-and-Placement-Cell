...
if(isset($_POST)) {
		...
		if(file_exists($_FILES['image']['tmp_name'])) { 
			...
			if($imageFileType == "jpg" || $imageFileType == "png")  {
				...
				if($_FILES['image']['size'] < 500000) { // File size is less than 5MB
					...
					header("Location: edit-coordinator.php");
					exit();
				}
			} else {
	...
	$sql = "UPDATE coordinator SET coordinatorname='$coordinatorname', website='$website', city='$city', state='$state', contactno='$contactno', aboutme='$aboutme'";

	if($uploadOk == true) {
		$sql = $sql . ", logo='$file'";
	}

	$sql = $sql . " WHERE id_coordinator='$_SESSION[id_coordinator]'";

	if($conn->query($sql) === TRUE) {
		$_SESSION['name'] = $coordinatorname;
