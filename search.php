...
if(isset($_GET['filter']) && $_GET['filter']=='city') {
  ...
  $sql = "SELECT * FROM coordinator WHERE city='$_GET[search]'";

  $result = $conn->query($sql);
  if($result->num_rows > 0) {
    while($row1 = $result->fetch_assoc()) {
      $sql1 = "SELECT * FROM job_post WHERE id_coordinator>='$row1[id_coordinator]' LIMIT $start_from, $limit";
         ...
         <div class="attachment-block clearfix">
                ...
                <div class="attachment-pushed">
                  ...
                  <div class="attachment-text">
                      <div><strong><?php echo $row1['coordinatorname']; ?> | <?php echo $row1['city']; ?> | role <?php echo $row['role']; ?> Years</strong></div>
                  </div>
                </div>
              </div>
  ...
  if($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      $sql1 = "SELECT * FROM coordinator WHERE id_coordinator='$row[id_coordinator]'";
                $result1 = $conn->query($sql1);
                if($result1->num_rows > 0) {
